...
export default async function CarrierPagosPage() {
  ...
  const { data: splits } = await db
    ...
    .order('due_date', { ascending: true }) as { data: PaymentSplit[] | null }
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      <Header title="Pagos pendientes" subtitle={`${pending.length} cobro${pending.length !== 1 ? 's' : ''} por recibir`} />
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-5">
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            { label: 'Por cobrar',    value: `$${totalPending.toLocaleString('en', { minimumFractionDigits: 2 })}`, suffix: 'USD', icon: DollarSign, color: 'text-[#06B6D4]',  bg: 'bg-cyan-50',  border: 'border-cyan-100' },
        ...
        {overdue > 0 && (
          <div className="flex items-center gap-3 bg-red-50 border border-red-200 rounded-xl px-4 py-3">
            ...
            <p className="text-sm text-red-700">Tienes <strong>{overdue} pago{overdue > 1 ? 's' : ''} vencido{overdue > 1 ? 's' : ''}</strong>. Contacta a tu Freight Forwarder.</p>
          </div>
        )}
                ...
                <TableRow>
                  <TableCell colSpan={5} className="py-20 text-center">
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <p className="text-sm text-slate-400">No tienes pagos pendientes</p>
                    </div>
                  </TableCell>
                </TableRow>
              ) : (
