...
export default async function TransaccionDetallePage({ params }: PageProps) {
  ...
  const [{ data: tx }, { data: events }, { data: duca }] = await Promise.all([
    ...
    db.from('duca_documents')
      .select('id, status, duca_number, tipo_duca, created_at')
      .eq('transaction_id', id)
      .eq('ff_user_id', user.id)
      .order('created_at', { ascending: false })
      .limit(1)
      .single() as Promise<{ data: { id: string; status: string; duca_number: string | null; tipo_duca: string; created_at: string } | null }>,
  ])
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-5">
        ...
        <div className="grid grid-cols-3 gap-5">
          ...
          <div className="col-span-2 space-y-5">
            ...
            {/* Timeline de tracking — el corazón del control tower */}
                  ...
                  <div className="relative">
                    ...
                    <div className="space-y-4">
                      {trackingList.map((event, i) => {
                        const cfg = EVENT_CONFIG[event.event_type] ?? { label: event.event_type, icon: MapPin, color: 'text-slate-500', dot: 'bg-slate-400' }
                        const Icon = cfg.icon
                        const isLast = i === trackingList.length - 1
                        return (
                          <div key={event.id} className="relative flex gap-4 pl-8">
                            {/* Dot */}
                            <div className={`absolute left-0 top-1 w-7 h-7 rounded-full ${isLast ? cfg.dot : 'bg-slate-200'} flex items-center justify-center shadow-sm z-10`}>
                            ...
                            <div className="flex-1 pb-1">
                              <div className="flex items-start justify-between gap-2">
                                <div>
                                  <p className={`text-sm font-semibold ${isLast ? cfg.color : 'text-slate-600'}`}>{cfg.label}</p>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white rounded-xl border border-slate-100 p-4">
              ...
              {lastEvent ? (
                <div>
                  {(() => {
                    const cfg = EVENT_CONFIG[lastEvent.event_type]
                    const Icon = cfg?.icon ?? MapPin
                    return (
                      <div className={`flex items-center gap-2 ${cfg?.color ?? 'text-slate-600'}`}>
                        <Icon size={16} />
                        <span className="text-sm font-semibold">{cfg?.label ?? lastEvent.event_type}</span>
                      </div>
                    )
                  })()}
              ...
              {duca ? (
                <div>
                  ...
                  <Badge variant="outline" className={`text-xs ${
                    ...
                    {duca.status === 'approved' ? 'Aprobada' : duca.status === 'submitted' ? 'En revisión' : 'Borrador'}
                  </Badge>
                </div>
              ) : (
                <div>
